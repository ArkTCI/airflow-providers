...
class FileMakerHook:
    ...
    def _get_conn_info(self) -> None:
        ...
        try:
            ...
            # If Airflow is not available, we can still use the hook directly
            pass
    ...
    def get_records(
        ...
        # Execute request
        return self.get_odata_response(
            endpoint=endpoint,
            params=params
        )
    ...
    def get_pool_info(self) -> Dict[str, str]:
        ...
        try:
            ...
            self.log.info(f"Retrieved pool info: Region={pool_info.get('Region')}, "
    ...
    def _authenticate_js_sdk_equivalent(self, username: str, password: str, mfa_code: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if 'ChallengeName' in response_json:
            ...
            if challenge_name in ['SMS_MFA', 'SOFTWARE_TOKEN_MFA']:
                ...
                raise AirflowException("Account requires password change. Please update password through the FileMaker Cloud portal.")
            else:
        ...
        self.log.info(f"Successfully obtained tokens. ID token first 20 chars: {auth_result.get('IdToken', '')[:20]}...")
        return auth_result
    
    def _respond_to_auth_challenge(self, username: str, challenge_name: str, mfa_code: str, challenge_response: Dict[str, Any]) -> Dict[str, Any]:
        """
